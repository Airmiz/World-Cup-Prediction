...
function renderPickList(){
  ...
  $("#plist").innerHTML=list.slice(0,220).map(p=>{
    ...
    const dis=have||full||!afford; const fm=p.form||0;
    ...
    return `<div class="pl ${dis?'dis':''}" data-pick="${p.id}"><div><span class="pos-tag pos-${p.pos}">${p.pos}</span><span class="pnm">${flag(p.team)} ${p.name}</span><div class="psub">${p.team}${full?' · 3 already':''}${have?' · in squad':''}${!afford?' · too expensive':''}</div></div><div class="ppr">£${p.price.toFixed(1)}</div>${metaR}</div>`;
...
function pick(id){ const p=F.byId[id]; if(!p)return;
  ...
  if(cur.length>=cap){ // remove cheapest same-pos to make room? better: tell user to remove
    // auto-replace the weakest same-position player (by projection pre-season, points once live)
    const dk=F.started?(x=>x.pts):(x=>x.xpts||0);
...
function autopick(){
  ...
  while(spent()>F.budget && guard++<80){
    ...
    for(const p of picked().sort((a,b)=>b.price-a.price)){
      const repl=F.players.filter(x=>x.pos===p.pos&&x.price<p.price&&!S.picks.includes(x.id)&&(x.team===p.team||teamCount(x.team)<F.max)).sort((a,b)=>((b.xpts+0.5)/b.price)-((a.xpts+0.5)/a.price))[0];
