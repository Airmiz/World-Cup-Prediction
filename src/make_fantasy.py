...
function genManagers(){
  ...
  // distinct nations for the two "national-core" rivals (drawn from the strongest sides present)
  ...
  // five genuinely different strategies — galácticos / differential / two national cores / big-budget —
  // so no two rivals (nor you) field the same XI. Small jitter keeps each auto-pick re-roll fresh.
  const styles=[
  ...
  for(let i=0;i<5;i++){ const sc0=styles[i]; const js=0.3+i*0.18;
    const jit={}; F.players.forEach(p=>jit[p.id]=(r()-0.5)*js);
    const m=p=>sc0(p)+(jit[p.id]||0);
    const sc=scoreTeam(optimizeSquad(m,F.budget));
...
function autopick(){
  ...
  const seed=(Date.now()^Math.floor(Math.random()*1e9))>>>0; _MGRS=null;
  // strongest squad that spends the budget on the highest-projected players (the stars)
  const picks=optimizeSquad(p=>p.xpts||0, F.budget);
  S={picks,cap:null,vice:null,bench:[],seed};
  normaliseXI();   // splits XI/bench by projection and sets the captain to the top projected starter
  save(); renderPitch(); renderPoints(); toast("Auto-picked your strongest XI — 5 fresh rivals drafted");
}
