...
<div class="wrap">
 <header class="hero">
  ...
  <p>Pick 15 players under a £100.0m budget, captain your star for double points, and score from real match data every matchday. Your squad saves to this device.</p>
 </header>
 <div class="bar" id="bar"></div>
 <div class="row" style="margin-bottom:8px">
  <button class="btn solid" id="autopick">✨ Auto-pick a team</button>
  <button class="btn" id="reset">Reset</button>
  <button class="btn" id="share">🔗 Share team</button>
...
function normaliseXI(){
  ...
  for(const k in byPos) byPos[k].sort((a,b)=>b.pts-a.pts||b.price-a.price);
  const xi=[byPos.GK[0]];
  ...
  const pool=[].concat(byPos.DEF.slice(FORM_MIN.DEF),byPos.MID.slice(FORM_MIN.MID),byPos.FWD.slice(FORM_MIN.FWD)).sort((a,b)=>b.pts-a.pts||b.price-a.price);
  ...
  if(!S.cap){ const best=xi.filter(p=>p.pos!=="GK").sort((a,b)=>b.price-a.price)[0]; if(best)S.cap=best.id; }
}
...
function renderBar(){
  ...
  +card(`${tot}`,"Total points","captain doubled")
  +card(`${F.gws.length?("MD"+F.gws[F.gws.length-1]):"—"}`,"Latest matchday","scored so far");
...
function jersey(p,fx,isBench){
  ...
  const val = fx? `<span class="spt fx">${flag(p.team)} £${p.price.toFixed(1)}</span>` : `<span class="spt">${p.pts}</span>`;
  return `<div class="slot" data-id="${p.id}"><div class="jersey">${p.num||p.pos[0]}${capBadge}</div><div class="snm">${flag(p.team)} ${p.name.split(" ").slice(-1)[0]}</div>${val}</div>`;
}
...
function renderPickList(){
  const pos=pickFor.pos; const q=$("#fsearch").value.toLowerCase(); const team=$("#fteam").value; const sort=$("#fsort").value;
  const rem=F.budget-spent();
  let list=F.players.filter(p=>p.pos===pos);
  ...
  list.sort(sort==="price"?(a,b)=>b.price-a.price : sort==="value"?(a,b)=>(b.pts/b.price)-(a.pts/a.price) : (a,b)=>b.pts-a.pts||b.price-a.price);
  $("#plist").innerHTML=list.slice(0,220).map(p=>{
    const have=S.picks.includes(p.id); const full=teamCount(p.team)>=F.max&&!have; const afford=p.price<=rem||have;
    const dis=have||full||!afford;
    return `<div class="pl ${dis?'dis':''}" data-pick="${p.id}"><div><span class="pos-tag pos-${p.pos}">${p.pos}</span><span class="pnm">${flag(p.team)} ${p.name}</span><div class="psub">${p.team}${full?' · 3 already':''}${have?' · in squad':''}${!afford?' · too expensive':''}</div></div><div class="ppr">£${p.price.toFixed(1)}</div><div class="ppt">${p.pts} pts</div></div>`;
  }).join("")||`<div class="empty">No players match.</div>`;
}
function pick(id){ const p=F.byId[id]; if(!p)return;
  if(S.picks.includes(id)){toast("Already in your squad");return;}
  if(teamCount(p.team)>=F.max){toast("Max 3 from "+p.team);return;}
  // replace an existing player of same pos if that position is full, else add
  const cap=F.squad[p.pos]; const cur=picked().filter(x=>x.pos===p.pos);
  if(cur.length>=cap){ // remove cheapest same-pos to make room? better: tell user to remove
    // auto-replace the lowest-points same-position player
    const drop=cur.sort((a,b)=>a.pts-b.pts||a.price-b.price)[0];
    S.picks=S.picks.filter(x=>x!==drop.id); if(S.cap===drop.id)S.cap=null; if(S.vice===drop.id)S.vice=null;
  }
  if(spent()+p.price>F.budget){ toast("Not enough budget"); // still allow but flag
  }
  S.picks.push(id); S.bench=[]; normaliseXI(); save(); closePicker(); renderPitch(); renderPoints(); toast(p.name+" added");
}
function removePlayer(id){ S.picks=S.picks.filter(x=>x!==id); if(S.cap===id)S.cap=null; if(S.vice===id)S.vice=null; S.bench=S.bench.filter(x=>x!==id); normaliseXI(); save(); renderPitch(); renderPoints(); }
...
function autopick(){
  ...
  for(const pos of POS){
    const cand=F.players.filter(p=>p.pos===pos).slice().sort((a,b)=>(b.pts+1)/b.price-(a.pts+1)/a.price);
    let got=0;
    for(const p of cand){ if(got>=need[pos])break; if((tc[p.team]||0)>=F.max)continue;
      const slotsLeft=15-S.picks.length; if(p.price>bud-(slotsLeft-1)*4.0)continue;
      S.picks.push(p.id); tc[p.team]=(tc[p.team]||0)+1; bud-=p.price; got++; }
    // fill remainder with cheapest if greedy fell short
    if(got<need[pos]) for(const p of cand.slice().sort((a,b)=>a.price-b.price)){ if(got>=need[pos])break; if(S.picks.includes(p.id))continue; if((tc[p.team]||0)>=F.max)continue; S.picks.push(p.id);tc[p.team]=(tc[p.team]||0)+1;bud-=p.price;got++; }
  }
  // guarantee within budget: downgrade the priciest player to the best cheaper same-position sub
  let guard=0;
  while(spent()>F.budget && guard++<80){
    let done=false;
    for(const p of picked().sort((a,b)=>b.price-a.price)){
      const repl=F.players.filter(x=>x.pos===p.pos&&x.price<p.price&&!S.picks.includes(x.id)&&(x.team===p.team||teamCount(x.team)<F.max)).sort((a,b)=>(b.pts/b.price)-(a.pts/a.price))[0];
      if(repl){ S.picks=S.picks.map(id=>id===p.id?repl.id:id); done=true; break; }
    }
    if(!done)break;
  }
...
function renderTable(){
  ...
  if(q)list=list.filter(p=>p.name.toLowerCase().includes(q)||p.team.toLowerCase().includes(q));
  list.sort((a,b)=>{const k=sortKey; let av=k==="value"?(a.pts/a.price):a[k], bv=k==="value"?(b.pts/b.price):b[k]; if(k==="name"||k==="team"){av=a[k];bv=b[k];return sortDir*(""+av).localeCompare(""+bv);} return sortDir*((bv-av));});
  const th=(k,l)=>`<th data-k="${k}">${l}${sortKey===k?(sortDir<0?" ▾":" ▴"):""}</th>`;
  $("#ptbl").innerHTML=`<thead><tr>${th("name","Player")}${th("team","Team")}${th("pos","Pos")}<th class="n" data-k="price">Price</th><th class="n" data-k="pts">Pts</th><th class="n" data-k="g">G</th><th class="n" data-k="a">A</th><th class="n" data-k="value">Value</th></tr></thead><tbody>`+
   list.slice(0,300).map(p=>`<tr><td>${flag(p.team)} ${p.name} ${S.picks.includes(p.id)?'<span style="color:var(--live)">●</span>':''}</td><td>${p.team}</td><td><span class="pos-tag pos-${p.pos}">${p.pos}</span></td><td class="n">£${p.price.toFixed(1)}</td><td class="n"><b>${p.pts}</b></td><td class="n">${p.g}</td><td class="n">${p.a}</td><td class="n">${(p.pts/p.price).toFixed(1)}</td></tr>`).join("")+`</tbody>`;
}
...
function bind(){
  ...
  $("#reset").onclick=()=>{if(confirm("Clear your squad?")){S={picks:[],cap:null,vice:null,bench:[]};save();renderPitch();renderPoints();}};
...
fetch("fantasy.json",{cache:"no-store"}).then(r=>r.json()).then(d=>{
  F.budget=d.budget; F.squad=d.squad; F.max=d.max_per_team; F.players=d.players;
  F.players.forEach(p=>F.byId[p.id]=p);
  const gws=new Set(); F.players.forEach(p=>{for(const g in (p.gw||{}))if(+g>0)gws.add(+g);}); F.gws=[...gws].sort((a,b)=>a-b);
  // team filter options
  ...
  load(); bind(); renderPitch(); renderPoints();
